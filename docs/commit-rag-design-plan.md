...
## 5. Phase 2：JetBrains插件

### 5.0 开工前先确认一件事：core现在有没有CLI入口

Phase 1里VS Code插件是直接`import`了`@commit-rag/core`当库用，没必要单独做CLI。Phase 2要靠子进程调用，所以**第一步得先去检查`packages/core`现在有没有导出一个真正可执行的CLI**（`package.json`里的`bin`字段+对应脚本）。如果没有，这不是"顺便加一下"的小事，是"没这个后面全部动不了"的前置依赖，得排进Day 1。CLI至少要有两个子命令，并且**都用JSON输出**（方便Kotlin那边解析，别用纯文本）：

- `commit-rag-cli index [--repo <path>]`
- `commit-rag-cli generate [--repo <path>]`

好消息是索引文件（`.commit-rag/index.json`）是仓库级别的，VS Code那边建过的索引，JetBrains插件在同一个仓库里可以直接复用，不用重新建一遍。

### 5.1 技术栈与脚手架

用JetBrains官方的 **IntelliJ Platform Plugin Template**（GitHub上`JetBrains/intellij-platform-plugin-template`）起项目，Kotlin+Gradle，不要手搓项目结构——模板已经把插件签名、版本兼容性声明、CI这些琐事处理好了。

### 5.2 UI集成点：不用再猜了，有真实开源插件可以直接参考

上一版计划里这块是"实现时现查"，现在有了更具体的路径：市面上已经有功能几乎一样的开源IntelliJ插件在正常维护，可以让Claude Code直接读它们的源码，学它们怎么把生成动作挂进VCS提交面板，而不是凭空猜扩展点名字：

- **`Blarc/ai-commits-intellij-plugin`**（GitHub，读git diff+调LLM生成commit message，功能设计和你这个几乎一模一样）
- **CommitGPT**（JetBrains Marketplace上的同类插件，同样是读diff调LLM生成）

这两个都是真实上架/在维护的插件，扩展点用法是能跑通的现成参考，比我这边给一个可能过时的扩展点名字靠谱得多。让Claude Code先clone `ai-commits-intellij-plugin`读一遍它怎么注册到提交面板，再照着这个思路接自己的CLI调用。

### 5.3 密钥存储：PasswordSafe

VS Code那边用的是`SecretStorage`，IntelliJ Platform对应的是`com.intellij.ide.passwordSafe.PasswordSafe`——这是一个存在很多年的标准API，专门用来存插件级别的敏感凭据（这里就是DeepSeek和DashScope两个key），不要图省事写进插件的普通settings持久化里。

### 5.4 子进程调用要留意的细节

- Kotlin这边用`ProcessBuilder`起`commit-rag-cli generate`，标准输入输出显式指定UTF-8编码（Windows下默认编码不一定是UTF-8，容易把中文commit message搞乱，这个坑值得提前避开）
- 这个方案要求本地有Node.js环境，这是已知代价——插件首次启动时应该检测Node是否可用、版本是否够，检测不到时给清晰的报错引导用户装Node，而不是静默失败
- 子进程调用有几百毫秒级的启动延迟，UI侧要有loading状态，别让用户以为卡死了

### 5.5 Day-by-day拆解

比Phase 1的拆解更粗一点，因为具体API要边查边定：

- **Day 1**：确认/补齐core的CLI入口，本地手动跑通`commit-rag-cli generate`能吐出正确的JSON
- **Day 2**：用官方模板起Kotlin插件脚手架，读一遍5.2提到的参考插件源码，定下这次要挂的具体UI挂载方式
- **Day 3**：把`ProcessBuilder`调CLI这部分打通，先不管UI，能在Kotlin测试里跑通"传一个diff进去，拿到解析好的JSON结果"就算过关
- **Day 4**：接入PasswordSafe做key存储，加首次使用的配置引导
- **Day 5-6**：UI集成（挂进commit面板），端到端手动测试，中英文diff都测一遍（防止5.4提到的编码坑）
- **Day 7（缓冲）**：README补JetBrains部分，打包成插件zip，装进一个真实的IntelliJ/Android Studio验证能跑通

### 5.6 顺带的一点

这一期做完之后，Kotlin/IntelliJ SDK的经验对你"全栈+AI辅助开发"这个定位也是个不错的补充——Kotlin是JVM生态语言，IntelliJ Platform开发本身涉及不少工程化的东西（Gradle构建、PSI、扩展点机制），跟你之前想靠近Java方向这件事不冲突，算是一个自然的搭桥。
...
4. **IntelliJ UI集成点**：已经不是纯靠现查了——第5.2节列了两个可以直接参考源码的开源插件，Phase 2开工先读它们的实现，比自己盲猜靠谱
